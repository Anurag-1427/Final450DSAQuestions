// T.C: O(R x C) -  Only one traversal of the matrix is required, so the time complexity is O(R X C)
// S.C: O(C) -  Stack is required to store the columns, so so space complexity is O(C)
